...
IplImage* convert_to_gray32( IplImage* img )
{
	...
	if( img->nChannels == 1 )
		gray8 = (struct _IplImage *)cvClone( img );
	else
							...
							int intvls, double sigma )
{
	...
	gauss_pyr = (struct _IplImage ***)calloc( octvs, sizeof( IplImage** ) );
	for( i = 0; i < octvs; i++ )
		gauss_pyr[i] = (struct _IplImage **)calloc( intvls + 3, sizeof( IplImage* ) );
...
IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
	...
	dog_pyr = (struct _IplImage ***)calloc( octvs, sizeof( IplImage** ) );
	for( i = 0; i < octvs; i++ )
		dog_pyr[i] = (struct _IplImage **)calloc( intvls + 2, sizeof(IplImage*) );
